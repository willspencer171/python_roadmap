...
"""
Task 5: Print all possible combinations of r elements in a given array of size n
For an array length n, find all combinations of length r
eg array [1,2,3,4] (n = 4), with r = 2, outputs are [1,2], [1,3], [1,4], [2,3], [2,4], [3,4]
"""

def poss_arr_outputs(array, r):

    data = [0] * r

    n = len(array)

    def utility(array, data, start, end, index, r):
        # wonder if this works in trying to get something out from deep in recursion
        if index == r:
            yield tuple(data)
            return
        # Like instead of directly calling utility, I'll 
        # yield from it?
        
        i = start
        while i <= end and end - i + 1 >= r - index:
            data[index] = array[i]
            yield from utility(array, data, i + 1, end, index + 1, r)
            i += 1
            
    
    yield from utility(array, data, 0, n - 1, 0, r)

    
arr = [1, 2, 3]
#print(poss_arr_outputs(arr,2))
print([i for i in poss_arr_outputs(arr, 2)])
output = [i for i in poss_arr_outputs(arr, 2)]
ouput1 = list(poss_arr_outputs(arr, 2))
